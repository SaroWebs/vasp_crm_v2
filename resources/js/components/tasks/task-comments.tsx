...
export default function TaskComments({ taskId }: TaskCommentsProps) {
    ...
    const [deletedComments, setDeletedComments] = useState<TaskComment[]>([]);
    ...
    const basePath = '/data/tasks';

    const fetchComments = async (showLoading = true) => {
        try {
            if (showLoading) setLoading(true);
            setError(null);
            const response = await axios.get(`${basePath}/${taskId}/comments`);
    ...
    const fetchDeletedComments = async () => {
        ...
        try {
            const response = await axios.get(
                `${basePath}/${taskId}/comments/deleted`,
            );
            if (response.data.success) {
    ...
    const handleRestoreComment = async (commentId: number) => {
        try {
            const response = await axios.patch(
                `${basePath}/${taskId}/comments/${commentId}/restore`,
            );
            if (response.data.success) {
    ...
    const handleForceDeleteComment = async (commentId: number) => {
        ...
        try {
            const response = await axios.delete(
                `${basePath}/${taskId}/comments/${commentId}/force-delete`,
            );
            if (response.data.success) {
                setDeletedComments((prev) =>
                    prev.filter((c) => c.id !== commentId),
    ...
    const handleCommentUpdate = (
        updatedComment: TaskComment | TicketComment,
    ) => {
        setComments(
            comments.map((c) =>
                c.id === updatedComment.id
                    ? (updatedComment as TaskComment)
                    : c,
            ),
        );
    };
