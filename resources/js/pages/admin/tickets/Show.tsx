...
export default function TicketsShow({ ticket }: TicketsShowProps) {
    ...
    const [pendingAction, setPendingAction] = useState<'close' | 'reject' | 'cancel' | null>(null);
    ...
    const taskStats = useMemo(() => {
        ...
        return {
            ...
            inProgress: tasks.filter((t: Task) => t.state == 'InProgress').length,
    ...
    const handleReopenTicket = () => {
        setIsApproving(true);
        axios
            .patch(`/admin/tickets/${ticket.id}/status`, { status: 'in-progress' })
            .then((res) => {
                toast.success('Ticket reopened successfully!');
                window.location.reload();
            })
            .catch((err) => {
                toast.error('Failed to reopen ticket: ' + (err.response?.data?.message || err.message));
            })
            .finally(() => {
                setIsApproving(false);
            });
    ...
    const handleMarkInProgress = () => {
        ...
        axios
            ...
            .catch((err) => {
                toast.error('Failed to update ticket: ' + (err.response?.data?.message || err.message));
            })
            .finally(() => {
                setIsApproving(false);
            });
    ...
    const checkTasksBeforeAction = async (action: 'close' | 'reject' | 'cancel') => {
        setIsCheckingTasks(true);
        try {
            const response = await axios.get(`/admin/tickets/${ticket.id}/check-tasks`);
    ...
    const handleCancelTicket = () => {
        setIsApproving(true);
        axios
            .patch(`/admin/tickets/${ticket.id}/status`, { status: 'cancelled' })
            .then((res) => {
                toast.success('Ticket cancelled successfully!');
                window.location.reload();
            })
            .catch((err) => {
                toast.error('Failed to cancel ticket: ' + (err.response?.data?.message || err.message));
            })
            .finally(() => {
                setIsApproving(false);
            });
    ...
    const handleConfirmPermanentDelete = () => {
        setIsApproving(true);
        axios
            .delete(`/admin/tickets/${ticket.id}`, { data: { force_delete: true } })
            .then((res) => {
                console.log(res);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-6">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    ...
                                    {['open', 'approved', 'in-progress'].includes(ticket.status) && (
                                        <DropdownMenuItem
                                            onClick={() => checkTasksBeforeAction('close')}
                                        >
                                    ...
                                    {['closed', 'cancelled'].includes(ticket.status) ? (
                                        ...
                                        <DropdownMenuItem onClick={() => setShowUserAssignment(true)} >
                                    ...
                                    {['open', 'approved', 'in-progress'].includes(ticket.status) && (
                                        <DropdownMenuItem
                                            ...
                                            onClick={() => checkTasksBeforeAction('cancel')}
                                        >
                                    ...
                                    {ticket.status !== 'cancelled' && ticket.status !== 'closed' && (
                                        <DropdownMenuItem
                                            ...
                                            onClick={() => checkTasksBeforeAction('reject')}
                                        >
                                            <XCircle className="mr-2 h-4 w-4" />
                                            Reject Ticket
                                        </DropdownMenuItem>
                                    ...
                                    {ticket.status === 'open' && (!ticket.tasks || ticket.tasks.length === 0) && (
                                        <>
                                            <DropdownMenuSeparator />
                                            <DropdownMenuItem
                    ...
                    <div className="grid gap-6 lg:grid-cols-3">
                        ...
                        <div className="space-y-6 lg:col-span-2">
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="grid gap-4 md:grid-cols-2">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="pl-6">
                                                <p className="text-sm font-medium">
                                                    {ticket.organization_user?.name || 'Unknown User'}
                                                </p>
                                                {ticket.organization_user?.designation && (
                                                    <p className="text-xs text-muted-foreground">
                                                        {ticket.organization_user.designation}
                                                    </p>
                                                )}
                            ...
                            {(ticket.assigned_to || ticket.approved_by) && (
                                <Card>
                                    <CardHeader>
                                        <CardTitle>
                                            Assignment & Approval
                                        ...
                                        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span className="text-sm font-medium text-blue-800">
                                                    Ticket status is independent of task status
                                                </span>
                                            </div>
                                                ...
                                                >
                                                    <div className="flex items-start justify-between">
                                                        ...
                                                        <div className="flex items-center space-x-2">
                                                            ...
                                                            <Badge className={getPriorityColor(task.sla_policy?.priority || 'P2')} >
                                                            ...
                                                            {task.assigned_users && task.assigned_users.length > 0 && (
                                                                <div className="flex items-center gap-1">
                                                                    ...
                                                                    {task.assigned_users.length === 1 ? (
                                                                        <span className="text-xs">{task.assigned_users[0].name}</span>
                                                                    ) : (
                                ...
                                <CardContent className="space-y-4">
                                    {ticket.organization_user && (
                                        <>
                                            ...
                                            {ticket.organization_user.email && (
                                                <div className="space-y-1">
                                                    <span className="text-sm text-muted-foreground">
                                                        Email
                                                    </span>
            ...
            <Dialog open={showTaskCheckModal} onOpenChange={(open) => !open && setShowTaskCheckModal(false)}>
                        ...
                        <div className="flex items-center justify-between rounded-lg bg-muted p-4">
                            <div className="flex items-center gap-4">
                                <div className="text-center">
                                    <div className="text-2xl font-bold">{taskCheckData?.total_tasks || 0}</div>
                                    <div className="text-xs text-muted-foreground">Total</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-2xl font-bold text-green-600">{taskCheckData?.completed_tasks || 0}</div>
                                    <div className="text-xs text-muted-foreground">Completed</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-2xl font-bold text-orange-600">{taskCheckData?.incomplete_tasks || 0}</div>
                                        ...
                                        {task.is_completed ? (
                                            ...
                                            <div>
                                                <span className="text-orange-600">
                                                    Please ask {task.assignees?.[0]?.name || task.assigned_department?.name || 'the assignee'} to complete the task
                                                </span>
                                            </div>
                                        )}
                                    </div>
...
const UserAssignmentForm = ({
    ...
    return (
        <form onSubmit={handleSubmit} className="max-h-[70vh] overflow-y-auto px-2 py-4">
            <div className="space-y-4 ">
                <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                        <Label htmlFor="user-assignment">Assign Ticket To</Label>
                        <Select
                            ...
                            <div className="rounded-md border bg-muted/40 p-3 text-xs">
                                <div className="mb-2 flex items-center justify-between gap-2">
                                    <div>
                                        ...
                                        <p className="text-muted-foreground">{selectedUser.department_name || selectedUser.email}</p>
                                    </div>
                                    <Badge variant={getLoadBadgeVariant(selectedUser.load_status)}>
                                        {selectedUser.load_status === 'busy' ? 'Busy' : 'Free'}
                                    </Badge>
                                </div>
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="rounded border bg-background p-2">
                                        <p className="text-muted-foreground">Active</p>
                                        <p className="font-semibold">{selectedUser.active_task_count}</p>
                                    </div>
                                    <div className="rounded border bg-background p-2">
                                        <p className="text-muted-foreground">Working</p>
                                        <p className="font-semibold">{selectedUser.in_progress_task_count}</p>
                                    </div>
                                    <div className="rounded border bg-background p-2">
                                        <p className="text-muted-foreground">Pending</p>
                                        <p className="font-semibold">{selectedUser.pending_task_count}</p>
                                    </div>
                                </div>
