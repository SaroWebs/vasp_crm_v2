...
import { Button, Select as MantineSelect, TextInput } from '@mantine/core';
...
const exportColumns: ExportColumn[] = [
    ...
    { key: 'tasks_count', label: 'Tasks Count' },
    { key: 'comments_count', label: 'Comments Count' },
...
export default function TicketsIndex(props: TicketsIndexProps) {
    ...
    const handleExport = async (config: ExportConfig) => {
        ...
        try {
            ...
            if (config.filters.client_id) {
                params.client_id = config.filters.client_id;
            }
            ...
            const filename =
                filenameMatch?.[1] ??
                `tickets_${config.startDate}_to_${config.endDate}.csv`;
    ...
    const applyFilters = (
        ...
        setIsLoading(true);
        const search =
            'search' in overrides ? overrides.search : debouncedSearch;
        const status = 'status' in overrides ? overrides.status : statusFilter;
        const client =
            'client_id' in overrides ? overrides.client_id : clientFilter;
        const nextOrderBy =
            'order_by' in overrides ? overrides.order_by : orderBy;
        const nextOrderDirection =
            'order_direction' in overrides
                ? overrides.order_direction
                : orderDirection;
        router.get(
            '/admin/tickets',
            {
    ...
    const toggleSort = (field: SortField) => {
        ...
        const nextDirection =
            isSameField && orderDirection === 'asc' ? 'desc' : 'asc';
        setOrderBy(field);
        ...
        return (
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <div className="flex cursor-pointer flex-col gap-1 hover:opacity-80">
                        <Badge
                            variant={variants[workStatus.status] || 'outline'}
                            className={`flex w-fit items-center gap-1 ${colors[workStatus.status] || ''}`}
                        >
                            <Icon className="h-3 w-3" />
                            {workStatus.label}
                        </Badge>
                            ...
                            <DropdownMenuItem key={task.id} asChild>
                                ...
                                >
                                    <div className="flex w-full items-center justify-between">
                                        ...
                                        <span
                                            className={`rounded px-1.5 py-0.5 text-xs ${getTaskStateBadge(task.state)}`}
                                        >
                                            {task.state}
                                        </span>
                                    </div>
    ...
    const getTicketStatusBadge = (status: Ticket['status']) => {
        ...
        return (
            <Badge
                ...
                className={`flex items-center gap-1 ${config?.bgColor || ''} ${config?.textColor || ''}`}
            >
                <Icon className="h-3 w-3" />
                {config?.label || status}
            </Badge>
    ...
    const getClosedOnLabel = (ticket: Ticket) => {
        ...
        return new Date(ticket.latest_closed_history.created_at).toLocaleString(
            'en-US',
            {
                year: 'numeric',
                month: 'short',
    ...
    const wizCards = stats ? [
        ...
        {
            title: 'Open Today',
            text: "Today's opened tickets",
            stats: stats.open_today,
            icon: Clock,
            color: 'blue',
            link: '/admin/tickets?status=open',
        },
        {
            ...
            icon: Clock,
            color: 'purple',
            link: '/admin/tickets?status=in-progress',
        },
        {
            title: 'Completed',
            text: 'Closed tickets',
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                <MantineSelect
                                    ...
                                    clearable
                                />
                                <AdminRaiseTicket clients={clients} />
                                                                ...
                                                                {(
                                                                    ...
                                                                    ticket.status,
                                                                        ...
                                                                        <DropdownMenuItem
                                                                            ...
                                                                            className="flex cursor-pointer items-center gap-2 text-orange-600 focus:text-orange-600"
                                                                        >
                                                                            <X className="h-4 w-4" />
                                                                            {closingTicketId ===
                                                                                ticket.id
            ...
            <ExportModal
                ...
                initialStatuses={
                    statusFilter !== 'all'
                        ? [statusFilter]
                        : ticketStatusOptions.map((status) => status.value)
                }
                initialFilters={{
